...
export default function AllUsers({
  ...
  useEffect(() => {
    console.log("Printing users before setting non-contacts:",users);
    setNonContacts(
      users.filter(
  ...
  const handleNewChatRoom = async (user) => {
    console.log("Creating chat room between", currentUser.uid, "and", user.uid);
    const members = [currentUser.uid, user.uid];
    console.log("Members array:", members);
    const res = await createOrGetChatRoom(members);
